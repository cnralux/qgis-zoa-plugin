...
class ImportDxfDialog(QDialog, FORM_CLASS, Importer):
    ...
    def _loadQgisLayers(self):
        ...
        # Adds the ZOA map layers
        layers = [layer for layer in QgsProject.instance().mapLayers().values()]
        #for layer in ZOALuxembourg.main.qgis_interface.legendInterface().layers():
        for layer in layers:
            if layer.type() == QgsMapLayer.VectorLayer and ZOALuxembourg.main.current_project.isZOALayer(layer):
    ...
    def _getQgisLayersCombobox(self, selected_layer = None):
        ...
        for layer in self.qgislayers:
            layers[ZOALuxembourg.main.current_project.getLayerTableName(layer)]=layer.name()
    ...
    def _updateAndGetFieldsMapping(self, layersmapping_rowindex = None):
        ...
        qgis_tablename = ZOALuxembourg.main.current_project.getLayerTableName(qgis_layer)
    ...
    def _getQgisLayerFromTableName(self, tablename):
        ...
        for layer in self.qgislayers:
            if ZOALuxembourg.main.current_project.getLayerTableName(layer) == tablename:
                return layer
    ...
    def _launchImport(self):
        ...
        progressMessageBar = ZOALuxembourg.main.qgis_interface.messageBar().createMessage(QCoreApplication.translate('ImportDxfDialog','Importing DXF'))
