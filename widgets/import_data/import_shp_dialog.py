...
class ImportShpDialog(QDialog, FORM_CLASS, Importer):
    def __init__(self, filename, parent=None):
        ...
        if not self.shplayer.isValid():
            ZOALuxembourg.main.qgis_interface.messageBar().pushCritical(
    ...
    def _loadQgisLayers(self):
        ...
        for layer in layers:
            if layer.type() == QgsMapLayer.VectorLayer and layer.geometryType() == self.shplayer.geometryType() and ZOALuxembourg.main.current_project.isZOALayer(layer):
    ...
    def _loadMapping(self):
        ...
        for layer in self.qgislayers:
            if ZOALuxembourg.main.current_project.getLayerTableName(layer) == self.mapping.destinationLayerName():
                qgis_layer = layer
        ...
        for field in qgis_fields:
            # Skip PK field
            if field.name() == ZOALuxembourg.project.PK:
                continue
            # Skip IMPORT_ID field
            if field.name() == ZOALuxembourg.project.IMPORT_ID:
                continue
    ...
    def _updateMappingFromUI(self, mapping_rowindex = None):
        ...
        newmapping.setDestinationLayerName(ZOALuxembourg.main.current_project.getLayerTableName(qgis_layer))
    ...
    def _launchImport(self):
        ...
        progressMessageBar = ZOALuxembourg.main.qgis_interface.messageBar().createMessage(QCoreApplication.translate('ImportShpDialog', 'Importing {}').format(self.shplayer.source()))
        progress = QProgressBar()
        progress.setMaximum(self.shplayer.featureCount())
        progress.setAlignment(Qt.AlignLeft|Qt.AlignVCenter)
        progressMessageBar.layout().addWidget(progress)
        ZOALuxembourg.main.qgis_interface.messageBar().pushWidget(progressMessageBar, 0) # QGis.Info = 0
