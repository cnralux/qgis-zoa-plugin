...
from ZOALuxembourg.controls.filename import SimpleFilenamePicker
...
class Importer(object):
    ...
    def _commitImport(self):
        ...
        importlog_layer = ZOALuxembourg.main.current_project.getImportLogLayer()
        ...
        if not importlog_layer.commitChanges():
            ...
            ZOALuxembourg.main.qgis_interface.messageBar().pushCritical(QCoreApplication.translate('Importer', 'Error'),
                                                                        QCoreApplication.translate('Importer', 'Commit error on layer {}').format(importlog_layer.name()))
            errors = importlog_layer.commitErrors()
            for error in errors:
                QgsMessageLog.logMessage(error, 'ZOA Luxembourg', 2) # QGis.Critical = 2
        ...
        if self.imported_extent is not None:
            ZOALuxembourg.main.qgis_interface.mapCanvas().setExtent(self.imported_extent)

        ZOALuxembourg.main.qgis_interface.messageBar().clearWidgets()

        # Display features errors
        if len(self.features_errors) > 0:
            messageBar = ZOALuxembourg.main.qgis_interface.messageBar().createMessage(QCoreApplication.translate('Importer', 'Warning'),
            ...
            ZOALuxembourg.main.qgis_interface.messageBar().pushWidget(messageBar, 1) # Qgis.Warning = 1

        # Display commit errors
        for error in self.commit_errors:
            messageBar = ZOALuxembourg.main.qgis_interface.messageBar().createMessage(QCoreApplication.translate('Importer', 'Error'),
                                                                                      error)
            btnOpenLog = QPushButton(QCoreApplication.translate('Importer', 'Open log'))
            btnOpenLog.clicked.connect(ZOALuxembourg.main.qgis_interface.openMessageLog)
            messageBar.layout().addWidget(btnOpenLog)
            ZOALuxembourg.main.qgis_interface.messageBar().pushWidget(messageBar, 2) # Qgis.Critical = 2

        # Display success message
        if (len(self.features_errors) + len(self.commit_errors)) == 0:
            ZOALuxembourg.main.qgis_interface.messageBar().pushSuccess(QCoreApplication.translate('Importer', 'Success'),
    ...
    def _importLayer(self, src_layer, dst_layer, mapping, progressbar = None):
        ...
        importid_index = dst_layer.fields().indexFromName(ZOALuxembourg.project.IMPORT_ID)
