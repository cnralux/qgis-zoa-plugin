...
class TopologyChecker(object):
    ...
    def run(self):
        ...
        if modification_zoa_layer is not None:
            ...
            if entity_count==1:
                ...
                ZOALuxembourg.main.qgis_interface.messageBar().pushMessage(QCoreApplication.translate('Topology','Information'),
                                                                   QCoreApplication.translate('Topology','There is 1 selected entity in ZOA MODIFICATION layer. You can now check topology'))
            elif entity_count==0:
                ZOALuxembourg.main.qgis_interface.messageBar().pushMessage(QCoreApplication.translate('Topology_no','Information'),
                                                                   QCoreApplication.translate('Topology_no','There is no selected entity in ZOA MODIFICATION layer. You can now check topology'))
            else:
                qgis.utils.iface.messageBar().pushMessage(QCoreApplication.translate('Topology_many', 'Information'),
                                                                   QCoreApplication.translate('Topology_many','There are {} selected entities in ZOA MODIFICATION layer. You can now check topology').format(entity_count))
        else :
    ...
    def _readTest(self, index):
        ...
        current_project = ZOALuxembourg.main.current_project
